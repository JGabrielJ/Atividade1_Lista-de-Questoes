def comprar_frutas(morango: float = 0, uva: float = 0):
    '''
    ...
    quantidade (em Kg) de uvas adquiridas e escreva o valor a ser pago pelo cliente.
    '''

    preco_mor = 2.50 if morango <= 5 else 2.20
    preco_uva = 1.80 if uva <= 5 else 1.50
    val_total = morango * preco_mor + uva * preco_uva

    if morango + uva > 8 or val_total > 25.00:
        val_total -= val_total * 10 / 100
...
def test(obtido, esperado):
    ...
    print(f'{prefixo} Esperado: {repr(esperado)} \tObtido: {repr(obtido)}\033[1;m')
